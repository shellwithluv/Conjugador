...
# Título de la página
st.markdown("<h1 class='title'>CONJUGADOR DE VERBOS EN AIMARA</h1>", unsafe_allow_html=True)

# Crear una barra lateral con opciones
opcion = st.sidebar.selectbox(
    "Navega por la información",
    ["Conjugador de verbos en aimara", "Historia sobre el aimara", "Por qué un conjugador de aimara", "Quiénes somos"]
)
...
elif opcion == "Por qué un conjugador de aimara":
    st.markdown("<h2 class='subtitle'>Por qué un conjugador de aimara</h2>", unsafe_allow_html=True)
    st.markdown("""
    <p class='description'>
...
if st.button('Más información sobre la lengua aimara'):
...
quechua = pd.ExcelFile('aimara.xlsx')
...
pronombres = pd.read_excel('pronombres_aimara.xlsx')
